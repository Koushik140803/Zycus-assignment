...
**Failure 1: Hallucinated Quotes and Ticket IDs**

During development on a self-hosted 3B-parameter model (Qwen2.5-3B), the
extraction step fabricated ticket IDs (for example, `TD-001` instead of real
...
- **Mitigation:** We isolated quote extraction from brief synthesis via
  ...
  Gemini model (`gemini-3.5-flash-lite`). This fully resolved the issue in
  testing: `T2-02` (verbatim quote and real ticket ID check) and `T2-06`
  (an account with zero recent tickets, testing whether risks get invented
  from nothing) both passed with a 1.0 quality score in the final
  `eval_report.json`, with zero fabricated quotes or ticket IDs across all
  Task 2 test cases. As a further, code-level safety net for production, not
  implemented here but the natural next step, we would reject any risk item
  at the API layer whose quote fails the verbatim check, rather than relying
...
- **Mitigation:** We raised `max_tokens` to 8192 on all three LLM calls
  ...
  API layer. This is not yet implemented here, but is the direct next step.
...
or incorrect product or team assignment.
...
- **Mitigation:** The knowledge base is chunked on Markdown horizontal
  ...
  windows. At scale, we would add hybrid retrieval (BM25 plus dense vectors)
  to catch keyword matches that pure embedding similarity can miss.

## 2. Latency vs. Quality

**Trade-off:** Task 2 uses a two-stage prompt chain, one call to extract
verbatim quotes and a second to synthesize the brief, instead of one combined
...
against a self-hosted model, so PII in ticket and account data does leave
our environment as part of each request.

To manage this, we would add a redaction middleware layer, such as Microsoft
...
(for example, `john@company.com` becomes `<EMAIL_1>`), and a local lookup
table would re-insert the real values into the final JSON response after
generation, so the LLM itself never sees raw PII while the TAM-facing output
still reads naturally. This redaction layer is not yet implemented in this
...
- **Breaks second:** FastAPI's default synchronous request handling. Every
  triage or summary call blocks on a live LLM round-trip; at 10x volume this
  creates a queue bottleneck and gateway timeouts.
- **The fix:** move ChromaDB to a persistent client or server deployment
  (or `pgvector` on Postgres), and refactor the API routes to async with a
  task queue (such as Celery or Redis) so ingestion and generation don't
